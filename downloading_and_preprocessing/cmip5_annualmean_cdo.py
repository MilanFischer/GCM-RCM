...
L = logging.getLogger("cmip5_annualmean_cdo")
...
DST_ROOT = os.path.join(BASE_CMIP5, "annual_nc")  # <- annual means go here
...
def output_name(var: str, model: str, scenario: str, yr0: int, yr1: int) -> str:
    # yearmean time series (one value per year)
    return f"{var}_Amon_{model}_{scenario}_yearmean_{yr0}-{yr1}.nc"

def annualmean_for_var(
    src_files: List[str],
    out_file: str,
    year0: int,
    year1: int,
    ...
    try:
        if dry_run:
            ...
            L.info(f"  [dry-run] would selyear {year0}/{year1} and yearmean -> {out_file}")
            return
        ...
        # 3) annual mean (per year)
        L.info(f"  - yearmean -> {out_file}")
        ensure_dir(os.path.dirname(out_file))
        run_cdo(cdo_opts + ["yearmean", sel, out_file])

        # Optional sanity check (uncomment for strict coverage):
        # subprocess.run(["cdo", "-s", "showyear", out_file], check=True)
        # subprocess.run(["cdo", "-s", "-outputkey,ntime", "-info", out_file], check=True)
...
def main():
    ap = argparse.ArgumentParser(
        description="Compute CMIP5 annual means (yearmean) for hfls and vpd using CDO (subprocess)."
    )
    ap.add_argument("--src-root", default=SRC_ROOT, help="Source base: .../CMIP5/monthly_nc")
    ap.add_argument("--dst-root", default=DST_ROOT, help="Destination base: .../CMIP5/annual_nc")
    ...
    for scenario in args.scenarios:
        ...
        for model in models:
            ...
            for var in args.vars:
                ...
                try:
                    annualmean_for_var(files, out_file, yr0, yr1, args.overwrite, args.dry_run)
...
# Example usage:
# python cmip5_annualmean_cdo.py                       # process both scenarios with defaults
# python cmip5_annualmean_cdo.py --dry-run             # show planned operations only
# python cmip5_annualmean_cdo.py --overwrite           # recompute and overwrite outputs
# python cmip5_annualmean_cdo.py --scenarios historical --vars hfls  # only historical hfls
