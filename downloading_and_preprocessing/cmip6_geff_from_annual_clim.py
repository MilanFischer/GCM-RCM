...
L = logging.getLogger("cmip6_geff_from_annual_clim")
...
# ---- paths (CMIP6) ----
BASE_CMIP6 = "/mnt/data/Other/GCM-RCM/GCM/CMIP6"
SRC_ROOT = os.path.join(BASE_CMIP6, "annual_nc", "ScenarioMIP")   # source: single-layer climatologies
DST_ROOT = os.path.join(BASE_CMIP6, "annual_nc", "ScenarioMIP")   # write alongside
...
def find_file(model_dir: str, var: str, scenario: str):
    """Prefer *_AyrClim_*; fallback to *_yearmean_*; robust to scenario case."""
...
def _standardize_coords(da: xr.DataArray) -> xr.DataArray:
    ...
    if "lat" in da.coords:
        da = da.sortby("lat")
        try: da = da.assign_coords(lat=np.round(da["lat"].astype(float).values, 6))
        except Exception: pass
    if "lon" in da.coords:
        da = da.sortby("lon")
        try: da = da.assign_coords(lon=np.round(da["lon"].astype(float).values, 6))
        except Exception: pass
    return da

def _align_or_regrid(vpd: xr.DataArray, hfls: xr.DataArray, regrid_method: str = "nearest"):
    """Try inner align; if empty overlap, regrid hfls to vpd grid and retry."""
...
def compute_geff(vpd: xr.DataArray, hfls: xr.DataArray) -> xr.DataArray:
    ...
    geff.attrs.update({
        ...
        "expects": "vpd in kPa, hfls in W m-2; single-layer climatologies",
    })
    return geff
...
def main():
    ap = argparse.ArgumentParser(
        description="Compute geff from CMIP6 single-layer annual climatologies of vpd & hfls (ScenarioMIP)."
    )
    ap.add_argument("--src-root", default=SRC_ROOT, help=".../CMIP6/annual_nc/ScenarioMIP")
    ap.add_argument("--dst-root", default=DST_ROOT, help=".../CMIP6/annual_nc/ScenarioMIP (outputs here)")
    ap.add_argument("--scenarios", nargs="+", default=SCENARIOS)  # historical, ssp585
...
# Example usage:
# python cmip6_geff_from_annual_clim.py
# python cmip6_geff_from_annual_clim.py --dry-run
# python cmip6_geff_from_annual_clim.py --overwrite
# python cmip6_geff_from_annual_clim.py --scenarios ssp585
