...
output_base = os.path.join(working_dir, "GCM", "CMIP5_4_EURO-CORDEX")
...
for model in models:
    for scenario in scenarios:
        ...
        base_path = f"/mnt/k_drive/Data/GCM/CMIP5_4_EURO-CORDEX/daily_nc/{scenario}/{model}/"
        ...
        for (variant, grid, timerange), files in file_index.items():
            if all(var in files for var in ["tasmin", "tasmax", "huss", "psl"]):
                ...
                # Load data
                huss_ds = xr.open_dataset(local_files["huss"])
                psl_ds = xr.open_dataset(local_files["psl"]) # Note we should use ps
