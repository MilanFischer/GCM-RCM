...
L = logging.getLogger("cmip6_annual_clim_cdo")

# -----------------------
# Paths & Defaults (CMIP6)
# -----------------------
BASE_CMIP6 = "/mnt/data/Other/GCM-RCM/GCM/CMIP6"
# monthly source is under ScenarioMIP
SRC_ROOT = os.path.join(BASE_CMIP6, "monthly_nc", "ScenarioMIP")
# write single-layer annual climatologies under annual_nc/ScenarioMIP
DST_ROOT = os.path.join(BASE_CMIP6, "annual_nc", "ScenarioMIP")

SCENARIOS = {
    "historical": {"years": (1981, 2005)},
    "ssp585": {"years": (2076, 2100)},
}
...
def find_var_files(model_dir: str, var: str, scenario: str) -> List[str]:
    """
    Find CMIP6 monthly files for a variable inside model_dir.
    Robust to: case of scenario token, minor naming differences, missing token fallback.
    """
...
def annual_clim_for_var(
    ...
    try:
        ...
        # sanity (optional):
        # subprocess.run(["cdo", "-s", "-outputkey,ntime", "-info", out_file], check=True)  # expect 1
...
def main():
    ap = argparse.ArgumentParser(
        description="Compute CMIP6 single-layer annual climatology (mean of annual means) for hfls and vpd using CDO."
    )
    ap.add_argument("--src-root", default=SRC_ROOT, help="Source: .../CMIP6/monthly_nc/ScenarioMIP")
    ap.add_argument("--dst-root", default=DST_ROOT, help="Dest:   .../CMIP6/annual_nc/ScenarioMIP (single-layer outputs)")
...
# Example usage:
# python cmip6_annual_clim_cdo.py                       # process historical + ssp585
# python cmip6_annual_clim_cdo.py --dry-run             # show plan only
# python cmip6_annual_clim_cdo.py --overwrite           # recompute everything
# python cmip6_annual_clim_cdo.py --scenarios historical --vars hfls  # only historical hfls
