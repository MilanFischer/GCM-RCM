...
L = logging.getLogger("cmip5_ymonmean_cdo")
...
DST_ROOT = os.path.join(BASE_CMIP5, "ymon_nc")

# Latest instruction: historical 1981–2005; RCP85 1976–2100
SCENARIOS = {
...
def find_var_files(model_dir: str, var: str, scenario: str) -> List[str]:
    ...
    Robust to:
    - lowercase/uppercase scenario tokens (RCP85 vs rcp85)
    - slightly different name positions
    - missing scenario token (fallback)
    """
    ...
    # Fallbacks (no scenario token) — as a last resort
    patterns += [
    ...
    # uniquify + sort for stable processing
    return sorted(set(files))

def output_name(var: str, model: str, scenario: str, yr0: int, yr1: int) -> str:
    return f"{var}_Amon_{model}_{scenario}_ymonmean_{yr0}-{yr1}.nc"

def ymonmean_for_var(
    src_files: List[str],
    out_file: str,
    year0: int,
    year1: int,
    ...
    # Work in the model directory for temp files
    ...
    try:
        if dry_run:
            L.info(f"  [dry-run] would mergetime {len(src_files)} files")
            L.info(f"  [dry-run] would selyear {year0}/{year1} and ymonmean -> {out_file}")
            return

        # NetCDF4 + compression on outputs
        cdo_opts = ["-f", "nc4c", "-z", "zip"]

        # 1) merge all time slices (if multiple)
        if len(src_files) == 1:
            # copy path reference through a local symlink to keep commands simple
            # but simplest: just treat as already merged
            L.info("  - single file: skipping mergetime")
            # still write a temporary merged to keep pipeline uniform
        ...
        # 2) select years
        # Note: selyear works with both monthly ranges and day-based ranges
        L.info(f"  - selyear {year0}/{year1} -> tmp_sel.nc")
        run_cdo(cdo_opts + [f"selyear,{year0}/{year1}", merged, sel])

        # 3) monthly climatology
        L.info(f"  - ymonmean -> {out_file}")
        ensure_dir(os.path.dirname(out_file))
        run_cdo(cdo_opts + ["ymonmean", sel, out_file])
...
def main():
    ap = argparse.ArgumentParser(
        description="Compute CMIP5 monthly climatologies (ymonmean) for hfls and vpd using CDO (subprocess)."
    )
    ap.add_argument("--src-root", default=SRC_ROOT, help="Source base: .../CMIP5/monthly_nc")
    ap.add_argument("--dst-root", default=DST_ROOT, help="Destination base: .../CMIP5/ymon_nc")
    ...
    for scenario in args.scenarios:
        ...
        for model in models:
            ...
            for var in args.vars:
                ...
                try:
                    ymonmean_for_var(files, out_file, yr0, yr1, args.overwrite, args.dry_run)
...
# Example usage:
# python cmip5_ymonmean_cdo.py                       # process both scenarios with defaults
# python cmip5_ymonmean_cdo.py --dry-run             # show planned operations only
# python cmip5_ymonmean_cdo.py --overwrite           # recompute and overwrite outputs
# python cmip5_ymonmean_cdo.py --scenarios historical --vars hfls # only historical hfls
